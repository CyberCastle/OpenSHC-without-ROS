...
void WalkController::generateLimits(StepCycle step,
    ...
    for (leg_it_ = model_->getLegContainer()->begin(); leg_it_ != model_->getLegContainer()->end(); ++leg_it_) {
        std::shared_ptr<Leg> leg = leg_it_->second;
        /// TODO: ROS_ASSERT(params_.offset_multiplier.data.count(leg->getIDName()));
...
StepCycle WalkController::generateStepCycle(const bool set_step_cycle) {
    ...
    /// TODO: ROS_ASSERT(step.stance_period_ % 2 == 0);
    /// TODO: ROS_ASSERT(step.swing_period_ % 2 == 0);
...
void WalkController::updateWalkPlane(void) {
    ...
    if (model_->getLegCount() >= 3) // Minimum for plane estimation
    {
        ...
        /// TODO: ROS_ASSERT(walk_plane_.norm() < UNASSIGNED_VALUE);
        /// TODO: ROS_ASSERT(walk_plane_normal_.norm() < UNASSIGNED_VALUE);
    } else {
        walk_plane_ = Eigen::Vector3d::Zero();
        walk_plane_normal_ = Eigen::Vector3d::UnitZ();
    }
...
LegStepper::LegStepper(std::shared_ptr<LegStepper> leg_stepper)
    : walker_(leg_stepper->walker_),
      leg_(leg_stepper->leg_), identity_tip_pose_(leg_stepper->identity_tip_pose_), default_tip_pose_(leg_stepper->default_tip_pose_), current_tip_pose_(leg_stepper->current_tip_pose_), origin_tip_pose_(leg_stepper->origin_tip_pose_), target_tip_pose_(leg_stepper->target_tip_pose_) {
...
void LegStepper::updateDefaultTipPosition(void) {
    ...
    /// TODO: ROS_ASSERT(new_default_tip_pose.isValid());
...
void LegStepper::updateTipPosition(void) {
    ...
    /// TODO: ROS_ASSERT(modified_stance_period != 0);
    ...
    if (step_state_ == SWING) {
        ...
        /// TODO: ROS_ASSERT(time_input <= 1.0);
        /// TODO: ROS_ASSERT(delta_pos.norm() < UNASSIGNED_VALUE);
