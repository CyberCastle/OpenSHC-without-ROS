...
Model::Model(const Parameters &params)
    : params_(params), leg_count_(static_cast<int>(params_.leg_id.data.size())), time_delta_(params_.time_delta.data), current_pose_(Pose::Identity()), default_pose_(Pose::Identity()) {
...
Model::Model(std::shared_ptr<Model> model)
    : params_(model->params_), leg_count_(model->leg_count_), time_delta_(model->time_delta_), current_pose_(model->current_pose_), default_pose_(model->default_pose_), imu_data_(model->imu_data_) {
}
...
Workplane Leg::getWorkplane(const double &height) {
    ...
    // TODO: ROS_ASSERT(workspace_.size() > 1);
...
void Leg::setDesiredTipPose(const Pose &tip_pose, bool apply_delta) {
    ...
    // TODO: ROS_ASSERT(desired_tip_pose_.isValid());
}
...
double Leg::updateJointPositions(const Eigen::VectorXd &delta, const bool &simulation) {
    ...
    for (joint_it = joint_container_.begin(); joint_it != joint_container_.end(); ++joint_it, ++index) {
        ...
        // TODO: ROS_ASSERT(joint->desired_velocity_ < UNASSIGNED_VALUE);
        ...
        // TODO: ROS_WARN_COND(!clamping_events.empty() && !params_.ignore_IK_warnings.data && !simulation,
...
double Leg::applyIK(const bool &simulation) {
    ...
    // TODO: ROS_ASSERT(position_delta.norm() < UNASSIGNED_VALUE);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (abs(position_error[i]) > IK_TOLERANCE) {
            ik_success = 0.0;
            // TODO: ROS_WARN_COND(!simulation && !params_.ignore_IK_warnings.data,
...
Pose Leg::applyFK(const bool &set_current, const bool &use_actual) {
    ...
    // TODO: ROS_ASSERT(current_tip_pose_.isValid());
...
Joint::Joint(std::shared_ptr<Leg> leg, std::shared_ptr<Link> reference_link,
             const int &id_number, const Parameters &params)
    : parent_leg_(leg), reference_link_(reference_link), id_number_(id_number), id_name_(leg->getIDName() + "_" + params.joint_id.data[id_number_ - 1] + "_joint"), min_position_(params.joint_parameters[leg->getIDNumber()][id_number_ - 1].data.at("min")), max_position_(params.joint_parameters[leg->getIDNumber()][id_number_ - 1].data.at("max")), offset_(params.joint_parameters[leg->getIDNumber()][id_number_ - 1].data.at("offset")), unpacked_position_(params.joint_parameters[leg->getIDNumber()][id_number_ - 1].data.at("unpacked")), max_angular_speed_(params.joint_parameters[leg->getIDNumber()][id_number_ - 1].data.at("max_vel")) {
...
Joint::Joint(std::shared_ptr<Joint> joint)
    : parent_leg_(joint->parent_leg_), reference_link_(joint->reference_link_), id_number_(joint->id_number_), id_name_(joint->id_name_), min_position_(joint->min_position_), max_position_(joint->max_position_), offset_(joint->offset_), packed_positions_(joint->packed_positions_), unpacked_position_(joint->unpacked_position_), max_angular_speed_(joint->max_angular_speed_) {
